...
lemma perim_subtriangle_is_half_triangle {T : Triangle}
  ...
  have sTa : (subTriangle T).a =  0.5 * (T.b + T.a) := by rfl
  have sTb : (subTriangle T).b =  0.5 * (T.c + T.b) := by rfl
  have sTc : (subTriangle T).c =  0.5 * (T.a + T.c) := by rfl
  rw[sTa,sTb,sTc]
  repeat rw[dist_eq_norm]
  repeat rw[dist_eq_norm] at h_perimT_eq_l

  calc ‖0.5 * (T.c + T.b) - 0.5 * (T.b + T.a)‖ + ‖0.5 * (T.a + T.c) - 0.5 * (T.c + T.b)‖ +‖0.5 * (T.b + T.a) - 0.5 * (T.a + T.c)‖ = ‖-1/2 *(T.b-T.a)‖ + ‖-1/2 *(T.c-T.b)‖ + ‖-1/2 *(T.a-T.c)‖  := by ring
